...
import { MiniApiTikTok } from "../MiniApiTikTok";
import { dataUsers } from "../data";

const App = () => {
  const dataTendencias = dataSetTendencias.discoverList;
  ...
  for (const video of MiniApiTikTok.video_list) {
    ...
    const thumbnailPath = video.cover_image_url.substring(
      video.cover_image_url.lastIndexOf("/") + 1
    );
  ...
  const contenidoPerfil = MiniApiTikTok;
  const infoPerfil = dataUsers;
  console.log("Informacion de las tendencias", dataTendencias);
  console.log(contenidoPerfil.video_list);
  console.log(infoPerfil.contentCreators);
